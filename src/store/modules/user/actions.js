...
const actions = {
    fetchUserList: async ({ state, commit, dispatch }, payload) => {
        ...
        try {
            ...
            let status = state.userList.filter.status === 999 ? '' : "|user_id.status:"+state.userList.filter.status
            ...
            const response = await http.get("/user/staff", {
                params: {
                    ...
                    conditions:'Or.name.icontains:'+search+'%2COr.code.icontains:'+search+status+area+division+role+warehouse +'|AndNot.user_id.status:3',
                    orderby:'-id',
                }
            });
